  ...
  "steps": [
    ...
    const renderDayCards = (renderDate) => {
        ...
        if (activeGoals.length === 0) {
            if (renderDate.toDateString() !== new Date().toDateString()) {
                return <p style={{textAlign:'center', marginTop:'30px', opacity: 0.5, lineHeight: '1.5'}}>День пуст.<br/>Не обманывай себя иллюзией отдыха.</p>;
            }
            return (
                <div className="ai-tactics-container">
                    <Icons.Cpu style={{ width: '32px', height: '32px', stroke: 'var(--accent)', marginBottom: '15px' }} />
                    <p style={{textAlign:'center', marginBottom:'25px', opacity: 0.7, lineHeight: '1.5', fontSize: '14px'}}>
                        День пуст. Жизнь идет.<br/>Ты создан созидать, а не наблюдать, как истекает время.<br/>Создай себе сопротивление.
                    </p>
                    
                    {!isAiScanning && !aiResult && (
                        <React.Fragment>
                            <h3 className="ai-tactics-title" style={{ fontSize: '18px', marginBottom: '15px' }}>Какая задача вызывает наибольшее сопротивление?</h3>
                            <input 
                                type="text" 
                                className="dark-input ai-tactics-input" 
                                placeholder="Например: Сесть за код, пойти в зал..." 
                                value={aiQuery} 
        ...
        return (
            <React.Fragment>
                <div className="daily-load-container">
                    <div className="load-header"><span className="load-title">Объем сопротивления</span><span className="load-count">{loadCount} {loadCount === 1 ? 'задача' : (loadCount > 1 && loadCount < 5) ? 'задачи' : 'задач'}</span></div>
