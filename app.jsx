...
const TimeWheel = ({ items, value, onChange, width = "60px" }) => {
    ...
    return (
        <div className="wheel-container" onScroll={handleScroll} ref={ref} style={{ width }}>
...
function App() {
    ...
    useEffect(() => {
        if (!daysInMonth.includes(startDay)) setStartDay('01');
    }, [daysInMonth]);
    ...
    const saveGoal = () => {
        ...
        // Если месяц уже прошел в этом году ИЛИ месяц тот же, но день прошел
        ...
        if (editingGoalId) setGoals(goals.map(g => g.id === editingGoalId ? { ...goalData, id: g.id, streak: g.streak, history: g.history, createdAt: g.createdAt } : g));
    ...
    const renderDayCards = (renderDate) => {
        ...
        const activeGoals = goals.filter(g => {
            if (!g.startDate) return true; // старые цели
            return new Date(g.startDate).getTime() <= renderTime;
        });
    ...
    return (
        <React.Fragment>
            ...
            <div className="container" style={{ paddingTop: isFullscreen ? 'calc(5px + 7vh)' : '10px' }}>
                ...
                {/* МОДАЛКИ (Редактирование/Удаление) */}
                {actionMenuGoal && (
                    ...
                    <div className="create-panel" style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        {createStep === 'time' && (
                            <div className="panel-step">
                                ...
                                {form.type === 'sprint' && (<input type="number" placeholder="Дней соблюдать?" value={form.duration} onChange={e => setForm({...form, duration: e.target.value})} className="dark-input" style={{textAlign: 'center', marginTop: '10px'}} />)}
                                
                                <hr className="divider" />
                                
                                {/* ДВОЙНОЕ КОЛЕСО (ВРЕМЯ И ДАТА НАЧАЛА) */}
                                <div className="wheels-grid">
                                    <div className="wheel-section">
                                        <div className="wheel-label">Дедлайн</div>
                                        <div className="ios-time-picker mini">
                                            <TimeWheel items={hoursList} value={form.deadline.split(':')[0]} onChange={h => setForm({...form, deadline: `${h}:${form.deadline.split(':')[1]}`})} width="40px" />
                                            <span className="time-colon">:</span>
                                            <TimeWheel items={minutesList} value={form.deadline.split(':')[1]} onChange={m => setForm({...form, deadline: `${form.deadline.split(':')[0]}:${m}`})} width="40px" />
                                        </div>
                                    </div>
                                    <div className="wheel-section">
