...
function App() {
    ...
    // МАГИЯ СИНХРОНИЗАЦИИ ЗАСТАВКИ И ВИБРАЦИИ
    useEffect(() => {
        const tg = window.Telegram?.WebApp;
        if (tg) { tg.ready(); tg.expand(); if (tg.initDataUnsafe?.user) setUserName(tg.initDataUnsafe.user.first_name); }

        // Тайминги вибрации строго подстроены под CSS-анимации огонька и вспышки
        const h1 = setTimeout(() => triggerHaptic('light'), 1000);  // Пульс 1 (борьба)
        const h2 = setTimeout(() => triggerHaptic('light'), 1800);  // Пульс 2
        const h3 = setTimeout(() => triggerHaptic('rigid'), 2600);  // Огонек сжимается в кулак
        const h4 = setTimeout(() => triggerHaptic('heavy'), 3000);  // ВЗРЫВ БЕЛОГО СВЕТА
        const h5 = setTimeout(() => triggerHaptic('success'), 3400); // Проявление логотипа

        // Полностью удаляем заставку из DOM через 4.5 секунды
        const endSplash = setTimeout(() => setShowSplash(false), 4500);

        return () => {
            clearTimeout(h1); clearTimeout(h2); clearTimeout(h3); 
            clearTimeout(h4); clearTimeout(h5); clearTimeout(endSplash);
        };
    }, []);
    ...
    const checkPermissions = (goal, checkDate) => {
        ...
        if (isToday) { const [h, m] = goal.deadline.split(':'); const limit = new Date(now); limit.setHours(parseInt(h, 10), parseInt(m, 10), 0, 0); if (now > limit) isDeadlinePassed = true; }
    ...
    const typeInfo = { once: { title: "Разовая", desc: "Сделать один раз и забыть." }, habit: { title: "Привычка (∞)", desc: "Регулярная задача." }, sprint: { title: "Спринт", desc: "Держись без срывов." } };
    ...
    const renderDayCards = (renderDate) => {
        ...
        if (goals.length === 0) return <p style={{textAlign:'center', marginTop:'20px', opacity: 0.7}}>Список пуст. Нажми + внизу!</p>;
    ...
    return (
        <React.Fragment>
            {/* ЭПИЧНАЯ ЗАСТАВКА (АНИМАЦИЯ СВЕТА И ВЗРЫВА) */}
            {showSplash && (
                <div className="epic-splash-overlay">
                    <div className="splash-darkness"></div>
            ...
            <div className="container" style={{ paddingTop: isFullscreen ? 'calc(20px + 7vh)' : '20px' }}>
                            ...
                            {[-1, 0, 1].map(shift => {
                                ...
                                return (
                                    <div className="cards-pane" key={renderDate.toDateString()}>
                                        <div style={{ textAlign: 'center', fontSize: '16px', fontWeight: 'bold', color: '#fff', height: '44px', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '10px' }}>
                                            {renderDate.toLocaleDateString('ru-RU', { day: 'numeric', month: 'long' })}
                                        </div>
                ...
                {actionMenuGoal && (
                    <div className="modal-overlay" onClick={() => setActionMenuGoal(null)}>
                        <div className="modal-content" style={{ paddingBottom: '40px', display: 'block' }} onClick={e => e.stopPropagation()}>
                            ...
                            <button className="btn-save" style={{background: '#000', marginBottom: '10px'}} onClick={() => { setForm({...actionMenuGoal, notifications: actionMenuGoal.notifications ?? true, supportTone: actionMenuGoal.supportTone ?? 'soft'}); setEditingGoalId(actionMenuGoal.id); setActionMenuGoal(null); setCreateStep('text'); setIsModalOpen(true); }}>✏️ Редактировать</button>
                    ...
                    <div className="create-panel" style={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        {createStep === 'text' && (
                            <div className="panel-step">
                                ...
                                <textarea placeholder="Описание (зачем тебе это?)" value={form.description} onChange={handleDescChange} className="dark-input custom-scrollbar" style={{ minHeight: '60px', maxHeight: '140px', resize: 'none', overflowY: 'auto' }} />
                            </div>
                        )}
                        ...
                        {createStep === 'time' && (
                            <div className="panel-step">
                                <div className="radio-group">
                                    <div className={`radio-btn ${form.type === 'once' ? 'active' : ''}`} onClick={() => {triggerHaptic('light'); setForm({...form, type: 'once'})}}><Icons.Target active={form.type === 'once'} /></div>
                                    <div className={`radio-btn ${form.type === 'habit' ? 'active' : ''}`} onClick={() => {triggerHaptic('light'); setForm({...form, type: 'habit'})}}><Icons.Infinity active={form.type === 'habit'} /></div>
                                    <div className={`radio-btn ${form.type === 'sprint' ? 'active' : ''}`} onClick={() => {triggerHaptic('light'); setForm({...form, type: 'sprint'})}}><Icons.Hourglass active={form.type === 'sprint'} /></div>
                                </div>
                                <div className="info-box">
                                    <div className="info-title">{typeInfo[form.type].title}</div><div className="info-desc">{typeInfo[form.type].desc}</div>
                                </div>
                                {form.type === 'sprint' && ( <input type="number" placeholder="Количество дней" value={form.duration} onChange={e => setForm({...form, duration: e.target.value})} className="dark-input" style={{textAlign: 'center', marginTop: '10px'}} /> )}
                                <hr className="divider" />
                                <div style={{textAlign: 'center', fontWeight: 'bold', marginBottom: '10px', fontSize: '14px'}}>Дедлайн</div>
                                <div className="ios-time-picker">
                                    <TimeWheel items={hoursList} value={form.deadline.split(':')[0]} onChange={h => setForm({...form, deadline: `${h}:${form.deadline.split(':')[1]}`})} /><span className="time-colon">:</span><TimeWheel items={minutesList} value={form.deadline.split(':')[1]} onChange={m => setForm({...form, deadline: `${form.deadline.split(':')[0]}:${m}`})} />
                                </div>
                                <div className="setting-row">
                                    <span style={{fontSize: '15px', fontWeight: '500'}}>Без выходных</span>
                                    <label className="ios-switch"><input type="checkbox" checked={form.ignoreHoliday} onChange={e => {triggerHaptic('light'); setForm({...form, ignoreHoliday: e.target.checked})}} /><span className="slider"></span></label>
                                </div>
                            </div>
                        )}
                        ...
                        {createStep === 'notifs' && (
                            <div className="panel-step">
                                <div className="setting-row" style={{marginBottom: '20px'}}>
                                    ...
                                    <label className="ios-switch"><input type="checkbox" checked={form.notifications} onChange={e => {triggerHaptic('light'); setForm({...form, notifications: e.target.checked})}} /><span className="slider"></span></label>
                                </div>
                                <hr className="divider" />
                                ...
                                <div className="radio-group" style={{ maxWidth: '200px', margin: '0 auto 10px auto' }}>
                                    <div className={`radio-btn ${form.supportTone === 'soft' ? 'active' : ''}`} onClick={() => {triggerHaptic('light'); setForm({...form, supportTone: 'soft'})}}><Icons.Heart active={form.supportTone === 'soft'} /></div>
                                    <div className={`radio-btn ${form.supportTone === 'hard' ? 'active' : ''}`} onClick={() => {triggerHaptic('light'); setForm({...form, supportTone: 'hard'})}}><Icons.Fire active={form.supportTone === 'hard'} /></div>
                                </div>
                ...
                <div className="tab-bar">
                    {!isModalOpen ? (
                        ...
                        <React.Fragment>
                            <div onClick={() => {triggerHaptic('light'); setCreateStep('text');}} className="tab-item"><Icons.Text active={createStep === 'text'} /></div>
                            <div onClick={() => {triggerHaptic('light'); setCreateStep('time');}} className="tab-item"><Icons.Clock active={createStep === 'time'} /></div>
                            <div className="tab-add-wrapper" onClick={closeCreateModal}><div className="tab-add-btn" style={{ background: '#444' }}><Icons.Add style={{ transform: 'rotate(45deg)', transition: 'transform 0.3s ease' }} /></div></div>
                            <div onClick={() => {triggerHaptic('light'); setCreateStep('notifs');}} className="tab-item"><Icons.Bell active={createStep === 'notifs'} /></div>
                            <div onClick={saveGoal} className="tab-item"><Icons.Save active={false} /></div>
                        </React.Fragment>
                    )}
                </div>
            </div>
